...
- rag：需要基于知识库资料回答，但通常一次检索即可完成。
- agent：需要多步工具调用，例如先检索再读取文档，再总结、生成、对比、分析。
...
约束：
1. 不确定时优先 chat 或 rag，不要滥用 agent。
2. 用户明确说“根据知识库/文档/资料”时优先 rag。
3. 用户需要多步读取、分析多个文档、生成方案时才 agent。
4. 只有需要外部系统时才 mcp。
